...
A = np.array([ data.x, np.ones(data.x.size)])
w = np.linalg.lstsq(A.T,data.z)[0] # obtaining the parameters
zline=w[0]*data.x+w[1]
zfixed = data.z-zline # substracting baseline from every point 
...
# Plot r
plt.figure(2);plt.clf() #name figure, clear plot
plt.plot(data.x[1:], r)
